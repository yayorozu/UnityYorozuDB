...
namespace Yorozu.DB.TreeView
{
    internal class YorozuDBEditorDataTreeView : UnityEditor.IMGUI.Controls.TreeView
    {
        ...
        internal YorozuDBEditorDataTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader,
            YorozuDBDataObject data) : base(state, multiColumnHeader)
        {
            _data = data;
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            if (GetSelection().Count > 1 &&
                _data.Define.KeyField != null &&
                _data.Define.KeyField.DataType == DataType.Int)
            {
                menu.AddItem(new GUIContent("Apply Sequential Number"), false,
                    () => AutoIdEvent?.Invoke(GetSelection()));
        ...
        internal static MultiColumnHeaderState CreateMultiColumnHeaderState(YorozuDBDataObject data)
        {
            ...
            foreach (var (field, i) in fields.Select((v, i) => (v, i)))
            {
                ...
                columns.Add(new MultiColumnHeaderState.Column()
                {
                    headerContent = data.Define.IsKeyField(field) ?
                        new GUIContent($"★ {field.Name}", field.Memo) :
                        new GUIContent($"    {field.Name}", field.Memo),
